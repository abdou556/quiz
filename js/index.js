...
class Question{
  constructor(index){
    ...
    this.correct_answers= questions[index].correct_answers;
  ...
  getAllAnswers(){
    let AllAnswers = [...this.incorrect_answers, this.correct_answers];
    AllAnswers.sort();
    return AllAnswers;
  }
  display(){
  ...
  checkAnswer(choice){
    if(!this.isAnswerd){
      this.isAnswerd= true
      if(choice.innerHTML == this.correct_answers){
        myQuiz.score++;
